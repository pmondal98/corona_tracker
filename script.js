$(document).ready(function() {
    ...
    var data_url = 'https://data.covid19india.org/v4/min/data.min.json';
    var timeSeries_url = 'https://data.covid19india.org/v4/min/timeseries.min.json'
    ...
    fetch(data_url)
        ...
        .then(data => {
            ...
            $("#confirmed").append(nFormatter(total_confirmed, total_confirmed.toString().length))
            $("#recovered").append(nFormatter(total_recovered, total_recovered.toString().length))
            $("#deaths").append(nFormatter(total_deaths, total_deaths.toString().length))
            $("#tested").append(nFormatter(total_tested, total_tested.toString().length))
            $("#vaccinated1").append(nFormatter(total_vaccinatedDose1, total_vaccinatedDose1.toString().length))
            $("#vaccinated2").append(nFormatter(total_vaccinatedDose2, total_vaccinatedDose2.toString().length))
          ...
          <div class="card-block">
             ...
             <p class="card-text">
                <div class="row text-center" style="color:#ffaa21">                        
                   ...
                   <h5 class="col-5" id="stateconfirmed">${nFormatter(Number(data[key].total.confirmed),Number(data[key].total.confirmed).toString().length)}</h5>                        
                </div>
                <div class="row text-center text-success">                        
                   <h5 class="col-7">Recovered</h5>
                   <h5 class="col-5" id="staterecovered">${nFormatter(Number(data[key].total.recovered),Number(data[key].total.recovered).toString().length)}</h5>                        
                </div>
                <div class="row text-center text-danger">                        
                   <h5 class="col-7">Deceased</h5>
                   <h5 class="col-5" id="statedeaths">${nFormatter(Number(data[key].total.deceased),Number(data[key].total.deceased).toString().length)}</h5>                        
                </div>   
                <div class="row text-center" style="color: yellowgreen;">                        
                   <h5 class="col-7">Tests</h5>
                   <h5 class="col-5" id="statetests">${nFormatter(Number(data[key].total.tested),Number(data[key].total.tested).toString().length)}</h5>                        
                </div>
                <div class="row text-center text-info">                        
                   <h5 class="col-7">Dose 1</h5>
                   <h5 class="col-5" id="dose1">${nFormatter(Number(data[key].total.vaccinated1),Number(data[key].total.vaccinated1).toString().length)}</h5>                        
                </div>
                <div class="row text-center text-info">                        
                   <h5 class="col-7">Dose 2</h5>
                   <h5 class="col-5" id="dose2">${nFormatter(Number(data[key].total.vaccinated2),Number(data[key].total.vaccinated2).toString().length)}</h5>                        
                </div>                                     
             </p>
          </div>
          <div class="card-footer">
             <small class="text-muted">Last updated at ${date_formatting(data[key].meta.last_updated)}</small>
          </div>                                    
          </div>
       </div>
    </div>  `
